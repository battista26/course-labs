...
int testhidemsg() {
    ...
    for (int i = 0; i < 100; i++) {
        unsigned char first = im->pixels[i].a & mask;
        unsigned char second = im->pixels[i].b & mask;
        unsigned char third = im->pixels[i].g & mask;
        unsigned char fourth = im->pixels[i].r & mask;
